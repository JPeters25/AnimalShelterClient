...
namespace AnimalShelterClient.Models
{
  class ApiHelper
  {
    ...
    public static async Task<string> GetSpecies(string species)
    {
      RestClient client = new RestClient("http://localhost:5000/api");
      RestRequest request = new RestRequest($"animals?species={species}", Method.GET);
      var response = await client.ExecuteTaskAsync(request);
      return response.Content;
    }

    public static async Task<string> GetGender(string gender)
    {
      RestClient client = new RestClient("http://localhost:5000/api");
      RestRequest request = new RestRequest($"animals?gender={gender}", Method.GET);
      var response = await client.ExecuteTaskAsync(request);
      return response.Content;
    }
